...
function BodyReader(options) {
    ...
    var xmlElementReaders = {
        ...
        "w:t": function(element) {
            return elementResult(new documents.Text(element.text(), element.attributes));
        },
    ...
    function readBlip(element, blip) {
        var docPrProperties = element.first("wp:docPr").attributes;
        var altText = isBlank(docPrProperties.descr) ? docPrProperties.title : docPrProperties.descr;

        var extentElement = element.first("wp:extent");
        var width = "";
        var height = "";

        if (extentElement && extentElement.attributes["cy"] && extentElement.attributes["cx"]) {
            width = String(Math.round(parseFloat(extentElement.attributes["cx"]) / 9525));
            height = String(Math.round(parseFloat(extentElement.attributes["cy"]) / 9525));
        }

        // eslint-disable-next-line no-console
        // console.log("-----------------------------------", width, height);
        return readImage(findBlipImageFile(blip), altText, width, height);
    }
